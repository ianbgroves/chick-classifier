# -*- coding: utf-8 -*-
"""
Created on Sat Feb  8 17:07:03 2020
...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  7 19:02:44 2020
...
print("Regularization status {}".format(regularization))
#%%
...
count = 0
lamda = 0.0001
lamdavar_iterate = [0.01, 0.001, 0.0001, 0.0005]
adam_lr = adam(lr = 0.0001)
#history_list = []
#%%
...
def check_reg(isReg, lamda, model, filters):
    
    if isReg:
         model.add(Conv2D(filters, (5, 5), activation ='relu', input_shape=(img_rows, img_cols, 1), kernel_regularizer=regularizers.l2(lamda)))
    else:
         model.add(Conv2D(filters, (5, 5), activation ='relu', input_shape=(img_rows, img_cols, 1)))
...
if test: 
    ...
    lamdavar_iterate = [0.0001]
for lamda in lamdavar_iterate:
        #initialize CNN | Convolution -> Pooling -> Flattening
    model = Sequential()
    #Convolution
    model = check_reg(regularization, lamda, model, 32)
    model.add(Dropout(0.1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    
    model = check_reg(regularization, lamda, model, 64)
    model.add(Dropout(0.1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    
    model = check_reg(regularization, lamda, model, 128) 
    model.add(Dropout(0.1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    
    model = check_reg(regularization, lamda, model, 256) 
    model.add(Dropout(0.1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    name = " Loss, Epochs, L2_Lamda {} ".format(epoch)
    plt.figure(name)
    ...
    #
    save_lamdavar = '/Users/Ian_b/Downloads/with_held_back_data.h5'
    model.save(save_lamdavar)  # creates a HDF5 file 'my_model.h5'
    ##del model  # deletes the existing model
    duration = time.time() - start
    print("{}s".format(round(duration,4)))
