...
for iterate in nb_filters:
    ...
    for epoch in epoch_iterate:
        ...
        model = Sequential()
        #Convolution
        model.add(Convolution2D(iterate, nb_conv, nb_conv,
                        border_mode='valid',
                        input_shape=(img_rows, img_cols,1)))
        convout1 = Activation('relu')
        #TODO: Add these convout1/2 variables as an argument in the convolution layers, rather than as separate model.add
        model.add(convout1)
        model.add(Convolution2D(iterate, nb_conv, nb_conv))
        ...
        #Connection
        model.add(Dense(128))
        #TODO: Add activation('relu')) to the dense layer statement
        model.add(Activation('relu'))
        model.add(Dropout(0.9))
        #Final Dense layer connects to the output neurons(?), three classess 10.1,2,3 so arg value 3
        model.add(Dense(nb_classes))
        #TODO: Add activation('softmax')) to the dense layer statement
        #TODO: Try sigmoid activation function rather than softmax
        ...
        name = " Accuracy, C filters:{} Epochs {}".format(iterate,epoch)
        plt.figure(name)
        ...
        name = " Loss, C filters:{} Epochs {}".format(iterate,epoch)
        plt.figure(name)
...
#
#model.save('/Users/Ian_b/Downloads/Cnn2.h5')  # creates a HDF5 file 'my_model.h5'
